-- Production-safe, server-authoritative child creation for independent-family
-- compatibility sessions. This narrow release intentionally does not depend on
-- public.admin_audit_events or any broader Admin/CRM audit subsystem.
--
-- Participant creation, family linking, and idempotency enforcement remain in
-- one database function invocation. Errors in those required operations are not
-- caught and therefore fail the operation instead of returning partial success.
...
begin
  ...
  -- Audit recording is deliberately omitted. The production schema does not have
  -- public.admin_audit_events, and child creation must not depend on the unrelated
  -- Admin/CRM audit subsystem. Required participant and family-link failures remain
  -- unhandled and abort this function invocation.
...
comment on function public.create_independent_family_child(text,text,text,text,text,text) is
  'Creates one participant and family link for a server-validated independent-family session without requiring the optional Admin/CRM audit subsystem.';
