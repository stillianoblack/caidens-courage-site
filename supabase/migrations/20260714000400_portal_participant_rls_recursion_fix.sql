-- Remove direct participants-table self-reference from its ownership policy.

create or replace function private.portal_can_access_participant(
...
begin
  ...
  return exists (
    ...
    where membership.auth_user_id = auth.uid()
      ...
      and (membership.valid_until is null or membership.valid_until > now())
      and (
        target_program_code = program.program_code
        or exists (
          select 1 from public.student_family_links link
          where link.student_id = target_participant_id
...
set row_security = off
as $$
declare target_program_code text;
begin
  select participant.program_code into target_program_code
  from public.participants participant where participant.id = target_participant_id;
  return private.portal_can_access_participant(target_participant_id, target_program_code);
end
$$;

revoke all on function private.portal_can_access_participant(uuid,text) from public, anon;
grant execute on function private.portal_can_access_participant(uuid,text) to authenticated, service_role;
