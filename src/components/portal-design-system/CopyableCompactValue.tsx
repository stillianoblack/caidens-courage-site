...
function EmailIcon() {
  return (
    <svg className="ds-copyChipIcon" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
...
export default function CopyableCompactValue({
  ...
  if (isEmptyValue(value)) {
    return <span className="ds-copyChip ds-copyChip--empty">—</span>;
  }
  ...
  return (
    <button
      type="button"
      className={`ds-copyChip ds-copyChip--${type}`}
      title={trimmed}
    ...
    >
      ...
      <span className="ds-copyChipText">{compactText}</span>
    </button>
  );
}
