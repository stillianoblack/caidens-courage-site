...
describe('CRM Phase 3 migration safety', () => {
  const sql = fs.readFileSync(path.resolve(process.cwd(), 'supabase/migrations/20260711000300_audience_crm_phase3_kit_automation.sql'), 'utf8'); const executable = sql.replace(/--.*$/gm, '');
