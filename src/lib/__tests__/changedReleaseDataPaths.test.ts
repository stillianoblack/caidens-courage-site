...
describe('changed release data-path safety', () => {
  ...
  test('the Focus Flame grade route consumes the validated participant roster', () => {
    const route = read('src/components/learning/QuestionBankRoute.tsx');
    expect(route).toContain('useActiveParticipant');
    expect(route).not.toContain(".from('participants')");
    expect(route).not.toContain('supabaseClient');
  });
