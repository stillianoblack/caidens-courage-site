...
describe('B-4 onboarding migration safety', () => {
  const preferenceMigration = fs.readFileSync(
    path.join(process.cwd(), 'supabase/migrations/20260715000100_b4_variant_preference.sql'),
    'utf8',
  );
  ...
  it('accepts legacy spark during the first migration before the later normalization tightens the constraint', () => {
    expect(preferenceMigration).toMatch(/check\s*\([^)]*'spark'[^)]*'courage'/i);
    expect(executable).toMatch(/set\s+b4_variant_key\s*=\s*'courage'\s+where\s+b4_variant_key\s*=\s*'spark'/i);
    expect(executable).toMatch(/check\s*\(b4_variant_key\s+in\s*\('courage',\s*'pattern',\s*'shield',\s*'anchor',\s*'fusion'\)\)/i);
  });
