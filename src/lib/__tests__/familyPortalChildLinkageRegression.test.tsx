...
describe('family child linkage regressions', () => {
  ...
  test('migration creates participant and family link atomically without placeholder names', () => {
    const sql = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260715000250_independent_family_child_creation_production_safe.sql'), 'utf8');
    expect(sql).toContain('create or replace function public.create_independent_family_child');
    expect(sql).not.toMatch(/(?:insert\s+into|from|join)\s+public\.admin_audit_events/i);
